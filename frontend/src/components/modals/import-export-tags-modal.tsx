...
import { X, Copy, Download, Check, FileCode, Upload, ArrowRight, Clipboard } from "lucide-react";
    ...
    <ModalContainer isOpen={isOpen} onClose={onClose} maxWidth="max-w-[540px]">
      ...
      {activeTab === "export" ? (
        <div className="space-y-3">
          ...
          <div className="p-3 bg-surface-l0 border border-theme-subtle rounded-md max-h-[220px] overflow-y-auto font-mono text-[12px] text-semantic-success whitespace-pre selection:bg-brand-accent/30">
            {exportedJson}
          </div>
          ...
          <div className="pt-2 flex items-center justify-end gap-2.5 border-t border-theme-subtle">
            ...
            <button
              type="button"
              onClick={handleCopy}
              className="px-3.5 py-2 bg-surface-l3 hover:bg-surface-l4 border border-theme-default text-theme-primary text-[13px] font-medium rounded-md flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-4 py-2 bg-brand-accent hover:bg-brand-accent-hover text-black text-[13px] font-medium rounded-md flex items-center gap-1.5 transition-colors"
            >
        ...
        <div className="space-y-3">
          ...
          <textarea
            ...
            className="w-full p-3 bg-surface-l0 border border-theme-subtle focus:border-brand-accent rounded-md text-[12px] font-mono text-theme-primary placeholder:text-theme-tertiary outline-none transition-colors resize-none overflow-y-auto"
          />

          <div className="border border-dashed border-theme-default hover:border-theme-strong rounded-md p-3 text-center bg-surface-l2 transition-colors cursor-pointer relative">
            <input
              type="file"
              accept=".json"
          ...
          {error && (
            <div className="text-[12px] text-semantic-danger bg-semantic-danger-subtle p-2 rounded-md border border-semantic-danger/20">
              {error}
            </div>
          )}
