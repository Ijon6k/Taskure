    ...
    <ModalContainer isOpen={isOpen} onClose={onClose} maxWidth="max-w-[420px]">
      ...
      <div className="space-y-2">
        {SHORTCUTS.map((item) => (
          <div
            key={item.key}
            className="flex items-center justify-between p-2.5 bg-[#18181A] border border-white/6 rounded-[8px] text-[13px]"
          >
            <span className="text-theme-primary/90 font-medium">{item.description}</span>
            <kbd className="px-2 py-1 bg-theme-elevated border border-white/12 rounded-[5px] text-[11px] font-mono text-brand-accent font-semibold shadow-xs">
              {item.key}
            </kbd>
          </div>
        ))}
      ...
      {/* Footer info */}
      <div className="pt-2 border-t border-white/6 text-center text-[11px] text-theme-tertiary">
        Press <kbd className="px-1 py-0.5 bg-theme-elevated rounded border border-white/10 text-theme-secondary">Esc</kbd> anytime to close
      </div>
    </ModalContainer>
  );
}
