...
export function ReorderColumnsModal({
  ...
  return (
    <ModalContainer isOpen={isOpen} onClose={() => !isSaving && onClose()} maxWidth="max-w-md">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-theme-default pb-3">
        ...
        <button
          onClick={onClose}
          disabled={isSaving}
          className="w-7 h-7 rounded-md flex items-center justify-center text-theme-secondary hover:text-theme-primary hover:bg-theme-elevated transition-colors disabled:opacity-30"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
