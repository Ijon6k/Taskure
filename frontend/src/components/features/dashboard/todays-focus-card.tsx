...
export function TodaysFocusCard({ focusData, loading }: TodaysFocusCardProps) {
  if (loading) {
    return (
      <div className="w-full bg-theme-surface border border-theme-default rounded-[10px] p-4 sm:p-5 animate-pulse space-y-4">
  ...
  if (!focusData || !focusData.task) {
    return (
      <div className="w-full bg-theme-surface border border-theme-default rounded-[10px] p-6 text-center space-y-2">
    ...
    <div className="w-full bg-theme-surface border border-theme-default rounded-[10px] p-4 sm:p-5 space-y-4 shadow-sm">
      ...
      <div>
        <Link
          ...
          className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-4 py-2.5 bg-brand-accent hover:opacity-90 active:scale-95 text-black text-[13px] sm:text-[14px] font-medium rounded-[7px] transition-all shadow-sm"
        >
